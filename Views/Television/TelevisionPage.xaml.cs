...
namespace PersonalTracker.Media.Views.MediaSeries
{
    ...
    public partial class TelevisionPage : Page, INotifyPropertyChanged
    {
        ...
        internal void RefreshItemsSource(List<Series> refreshSeries)
        {
            ...
            {
                _series = ChkAll.IsChecked.Value ? AppState.AllSeries.ToList() : refreshSeries;
        ...
        private void CheckRadio()
        {
            if (RadAll.IsChecked.Value)
                RefreshItemsSource(AppState.AllSeries.ToList().OrderBy(series => series.Name).ToList());
            else if (RadAiring.IsChecked.Value)
                RefreshItemsSource(_series = AppState.AllSeries.ToList().FindAll(series => series.Status == SeriesStatus.Airing).OrderBy(series => series.Day).ThenBy(series => series.Time).ThenBy(series => series.Name).ToList());
            else if (RadEnded.IsChecked.Value)
                RefreshItemsSource(_series = AppState.AllSeries.ToList().FindAll(series => series.Status == SeriesStatus.Ended).OrderBy(series => series.Name).ToList());
            else if (RadHiatus.IsChecked.Value)
                RefreshItemsSource(_series = AppState.AllSeries.ToList().FindAll(series => series.Status == SeriesStatus.Hiatus).OrderBy(series => series.Name).ToList());
            else if (RadNope.IsChecked.Value)
                RefreshItemsSource(_series = AppState.AllSeries.ToList().FindAll(series => series.Status == SeriesStatus.Nope).OrderBy(series => series.Name).ToList());
        }
